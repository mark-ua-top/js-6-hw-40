...
const RegisterPage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (password.length < 7) {
      toast.error('Пароль має бути не менше 7 символів');
      return;
    }
    dispatch(register({ name, email, password }))
      .unwrap()
      .then(() => toast.success('Ласкаво просимо! 🎉'))
      .catch((err) => toast.error(err || 'Помилка реєстрації'));
  };
      ...
      <div className="auth-card">
        <div className="auth-card-inner">
          <div className="auth-header">
            ...
            <p className="auth-subtitle">Приєднуйтесь — це безкоштовно</p>
          </div>

          <form className="auth-form" onSubmit={handleSubmit} autoComplete="off">
            <div className="input-group">
              <label className="input-label" htmlFor="reg-name">Ваше ім'я</label>
              <div className="input-wrap">
                <svg className="input-icon" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                <input
          ...
          <p className="auth-switch">
            Вже маєте акаунт?{' '}
            <Link to="/login" className="auth-switch-link">Увійти</Link>
          </p>
        </div>
      </div>
    </div>
